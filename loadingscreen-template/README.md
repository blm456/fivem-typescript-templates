...
No included in this template, but `ShutdownLoadingScreenNui()` must be called from the client when loaded, either on `playerSpawned` event, or starting a loop that waits for `NetworkIsSessionStarted()` to be true (with a few millisecond wait, like 10)
